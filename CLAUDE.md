...
### UI 交互规范
- **按钮文字禁止换行**：按钮文字必须单行显示，宁可缩短文案或缩小字号，绝不允许文字折行
- **标签/徽章文字禁止换行**：Tag、Badge 等标签类元素同理，必须单行
- **信息密度控制**：横向空间不足时优先精简文案，而非压缩间距
- **弹窗内容排版**：wx.showModal 的 content 需注意分段和换行，保持清晰的信息层次
