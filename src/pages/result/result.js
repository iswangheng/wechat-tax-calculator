// 12个月工资明细页面
const { calculateMonthlyTax, calculateYearlyTotal } = require('../../utils/tax-calculator');
const { drawPieChart, drawLineChart, initCanvas } = require('../../utils/chart-utils');
...
Page({
  ...
  onLoad(options) {
    ...
    const params = {
      ...
      socialInsurance: globalCalc.socialSecurity || parseFloat(options.social) || 0,
      specialDeduction: globalCalc.specialDeductions || parseFloat(options.deduction) || 0,
  ...
  renderSalaryDonut(ctx, width, height) {
    ...
    var socialIns = (typeof params.socialInsurance === 'object' && params.socialInsurance !== null)
    ...
    drawPieChart(ctx, width, height, data, monthlySalary.toFixed(0), '税前工资');
  },
  ...
  renderTaxTrend(ctx, width, height) {
    ...
    var labels = monthlyDetails.map(function(d) { return d.month + '月'; });
    var taxData = monthlyDetails.map(function(d) { return parseFloat(d.currentMonthTax) || 0; });
  ...
  calculateMonthlyDetails() {
    const { monthlySalary, socialInsurance, specialDeduction, currentMonth } = this.data.params;

    // Ensure socialInsurance is an object with .total for calculateMonthlyTax
    const socialSecurityParam = (typeof socialInsurance === 'object' && socialInsurance !== null)
      ? socialInsurance
  ...
  calculateTaxAnalysis(yearlyTotal) {
    ...
    const socialTotal = (typeof socialInsurance === 'object' && socialInsurance !== null)
      ? (socialInsurance.total || 0)
      : (socialInsurance || 0);
    const cumulativeTaxable = (monthlySalary - socialTotal - (specialDeduction || 0) - 5000) * currentMonth;
    ...
    const bracketsWithStatus = brackets.map(bracket => {
      const active = cumulativeTaxable >= bracket.min && cumulativeTaxable < bracket.max;
      if (active) {
    ...
    const effectiveRate = cumulativeTaxable > 0
      ? ((totalTax / cumulativeTaxable) * 100).toFixed(2)
      : '0.00';

    const avgMonthlyTax = currentMonth > 0
      ? (totalTax / currentMonth).toFixed(2)
      : '0.00';
    ...
    try {
      ...
      if (savedDeductions && savedDeductions.totalDeduction > 0) {
        ...
        if (savedDeductions.childCount > 0) {
          items.push({
            name: '子女教育',
            amount: savedDeductions.childCount * 1000 * (savedDeductions.childRatio / 100)
          });
        }
        ...
        if (savedDeductions.infantCount > 0) {
          items.push({
            name: '婴幼儿照护',
            amount: savedDeductions.infantCount * 1000 * (savedDeductions.infantRatio / 100)
          });
        }
        ...
        if (savedDeductions.hasElderCare) {
          const amount = savedDeductions.elderType === 'only'
            ? 2000
  ...
  drawCardContent(ctx, canvas) {
    ...
    const lastMonth = monthlyDetails.length > 0 ? monthlyDetails[monthlyDetails.length - 1] : null;

    // Extract socialInsurance numeric value
    const socialTotal = (typeof params.socialInsurance === 'object' && params.socialInsurance !== null)
      ? (params.socialInsurance.total || 0)
      : (params.socialInsurance || 0);
